  ...
  stages {
    ...
    stage('Deploy') {
        steps {
            ...
            sh "/usr/local/bin/aws --region ${region} ecs update-service --cluster ${cluster_arn}/${project_env}-ai --service ${project_env}-${module_name}-service --task-definition ${task_def_arn}/${project_env}-${module_name}-task"
        }
    }
    ...
    post {
      ...
      failure {
        ...
        emailext (
            to: 'devops;simplify_ai@simplifyworkforce.com',
